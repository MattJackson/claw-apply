...
Answer:`;
  ...
  async fill(page, resumePath, { modalSelector = null } = {}) {
    ...
    if (snap.resumeRadios.length > 0 && !snap.resumeChecked) {
      ...
      // Prefer resume-specific file inputs, skip autofill inputs
      const fileInput = await container.$('input[type="file"]#resume') ||
        await container.$('input[type="file"]#_systemfield_resume') ||
        await container.$('input[type="file"][id*="resume" i]') ||
      ...
      if (target) {
        ...
        if (acceptsResume) await target.setInputFiles(resumePath).catch(() => {});
      }
    }
    ...
    for (const field of snap.inputs) {
      ...
      // For number fields, always try AI if no answer — LinkedIn validates them even if not marked required
